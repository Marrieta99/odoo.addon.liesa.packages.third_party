...
class ProductDataFeedColumn(models.Model):
    ...
    @api.model
    def _get_installed_language_domain(self):
        return "[('id', 'in', %s)]" % self._get_installed_languages().ids
    ...
    relation_field_ttype = fields.Selection(related='relation_field_id.ttype')
    ...
    language_id = fields.Many2one(
        ...
        domain=lambda self: self._get_installed_language_domain(),
    ...
    def _compute_relation_model_id(self):
        for column in self:
            if column.type == 'field' and column.field_id.ttype in RELATION_TYPES + MULTI_RELATION_TYPES:
                column.relation_model_id = self.env['ir.model'].search([
                    ('model', '=', column.field_id.relation),
                ])[:1].id
            else:
        ...
        if isinstance(value, list):
            ...
            if column.format:
                if column.field_id.ttype in ['date', 'datetime']:
                    ...
                    lang_code = column.feed_id.lang or self.env.context.get('lang')
    ...
    def get_product_image_url(self, product, field_name: str=None, model_name: str=None, rec_id: int=None) -> str:
        self.ensure_one()
        feed = self.feed_id
        field_name = field_name or feed._get_product_image_field_name()
        model_name = model_name or feed.model_name
        rec_id = rec_id or product.id
        # flake8: noqa: E501
        return '%(base_url)s/web/image/%(model)s/%(id)d/%(field)s%(checksum)s' % {
            ...
            'field': field_name,
            'checksum': '%s' % (
                '/%s' % feed._get_image_checksum(product[field_name])
                if self.image_url_is_unique else ''
            ),
        }
        ...
        for image in images.filtered('image_1920'):
            image_links.append(
                ...
                ({
                    ...
                    'checksum': '%s' % (
                        '/%s' % feed._get_image_checksum(image[feed._get_product_image_field_name()])
                        if self.image_url_is_unique else ''
                    ),
                })
            )
    ...
    def get_product_taxes(self, product):
        ...
        return product.taxes_id.filtered(
            lambda tax: tax.company_id == self.feed_id.company_id
                        and (tax.country_id == self.feed_id.tax_country_id if self.feed_id.tax_country_id else True)
        )
    ...
    def get_special_value(self, product) -> str:
        ...
        elif column.special_type == 'seller_data':
            seller_data =  {
                'seller_id': feed.seller_id,
                'seller_name': feed.seller_id.name,
                'seller_url': feed.seller_url,
                'seller_privacy_url': feed.seller_privacy_url,
                'seller_terms_url': feed.seller_terms_url,
                'seller_return_url': feed.seller_return_url,
                'seller_return_days': str(feed.seller_return_days or ''),
            }
            if column.multi_dict_key and seller_data.get(column.multi_dict_key):
                value = seller_data[column.multi_dict_key]
            else:
                value = seller_data
    ...
    def _get_field_value(self, product, field_name=None, to_str=True):
        ...
        if column.field_id.ttype in RELATION_TYPES:
            ...
            if record:
                ...
                if value and column.relation_field_id.ttype == 'binary' and column.is_image_url:
                    value = column.get_product_image_url(
                        product, column.relation_field_id.name, model_name=record._name, rec_id=record.id,
                    )
    ...
    def _get_google_category(self, product):
        self.ensure_one()
        google_category = product.google_category_id
        if not google_category:
            for public_category in product.public_categ_ids:
                if public_category.google_category_id:
                    google_category = public_category.google_category_id
                    break
    ...
    def _get_value(self, product):
        ...
        if column.language_id and column.language_id in self._get_installed_languages():
